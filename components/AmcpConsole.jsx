  ...
  return (
    <div className="glass-panel p-5 mb-6 bg-white/90 dark:bg-slate-950/80 transition-colors duration-200">
      <div className="flex items-center justify-between gap-4 mb-3">
        ...
        <button
          onClick={onClearLogs}
          className="p-1.5 rounded text-slate-500 hover:text-slate-800 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all text-xs flex items-center gap-1"
      ...
      {/* Terminal Log Box */}
      <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 h-[240px] overflow-y-auto mono-font text-xs space-y-2 mb-3 shadow-inner">
        {logs.length === 0 ? (
      ...
      <form onSubmit={handleSubmit} className="flex gap-2">
        <div className="relative flex-1">
          <Cpu className="w-4 h-4 absolute left-3 top-2.5 text-slate-400 dark:text-slate-500" />
          <input
            type="text"
            value={customCmd}
